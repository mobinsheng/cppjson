 ...
 int strcasecmp(const char *str1,const char *str2)
{
 ...
 vector<xJSONNode*>& xJSONValue::toArray()
 {
     return m_ArrayValue;
 }

 xJSONNode*          xJSONValue::toObject()
 {
     return m_pObjValue;
 }
 ...
 xJSONValue& xJSONValue::operator=(const vector<xJSONNode*>& v)
 {
     m_ArrayValue = v;
     return *this;
 }

 xJSONValue& xJSONValue::operator=(xJSONNode* v)
 {
     m_pObjValue = v;
     return *this;
 }
...
const char* xJSON::ParseArray(xJSONNode* pNode,const char* buffer)
{
    ...
    if(*buffer == ']')
    {
        pNode->m_Value.toArray().push_back(node);
        ++buffer;
        return buffer;
    }
...
void xJSON::BuildArray(xJSONNode* pNode,string& strData,bool format ,int depth)
{
    ...
    for(int j = 0; j < pNode->GetValue().toArray().size(); ++j)
    {
        if(pNode->GetValue().toArray()[j] == 0)
            continue;

        xJSONNode* pChildNode = pNode->GetValue().toArray()[j];
        ...
        if(j != pNode->GetValue().toArray().size() - 1)
        {
            strData += ",";
        }
